...
vocself = False  # تعريف المتغير كقيمته الأولية
...
async def stop_datea(event):
    ...
    vocself = False
    await edit_or_reply(event, "**⎉╎تم تعطيل حفظ البصمة الذاتية 🎙**\n**⎉╎الان صارت مو شغالة .. ✅**")
...
async def sddm(event):
    ...
    if vocself:
        ...
        PM_LOGGER_GROUP_ID = ...  # تأكد من تحديد القناة المناسبة
        await zedub.send_file(PM_LOGGER_GROUP_ID, voc, caption=f"ᯓ 𝙏𝙀𝙋𝙏𝙃𝙊𝙉 ⌁ - حفـظ البصمة الذاتية 🎙\n⋆─┄─┄─┄─┄─┄─┄─⋆\n⌔ مࢪحبـًا .. عـزيـزي 🫂\n⌔ تـم حفظ البصمة الذاتية .. تلقائيًّـا ☑️ ❝\n⌔ معلومـات المـرسـل :-\n• الاسم : {_format.mentionuser(sender.first_name , sender.id)}\n• اليوزر : {username}\n• الايدي : {sender.id}")
