...
async def insta_dl(event):
    "عزيــزي استخـدم الأمـر بطريقـة صحيحــة."
    link = event.pattern_match.group(1)
    reply = await event.get_reply_message()
    if not link and reply:
        link = reply.text
    if not link:
        return await edit_delete(event, "**أعطـني الرابط..**", 10)
    if "instagram.com" not in link:
        return await edit_delete(
            event, "`أحتاج إلى رابط انستا لتحميل الفيديو الخاص به...`(*_*)", 10
        )
    # v1 = "@instasave_bot"
    # v1 = "@IgGramBot"
    v1 = "Fullsavebot"
    v2 = "@videomaniacbot"
    media_list = []
    catevent = await edit_or_reply(event, "**جــاري التحميــل ♥.....**")
