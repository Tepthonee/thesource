...
ZelzalVP_cmd = (
    ...
    "**⎉╎قائمـــة أوامـر تغييـر فـارات الصـور بأمـر واحـد فقـط - لـ اول مرة على سـورس تيبثـون يوزر بـوت 🦾 :** \n\n"
    ...
    "**⎉╎قائمـــة أوامـر تغييـر كليشـة الايـدي :** \n\n"
    ...
    "**⎉╎قائمـــة أوامـر تغييـر بقيـة الفـارات بأمـر واحـد فقـط :** \n\n"
    ...
    "⪼ `.حذف فار المطورين`\n\n"
    "**⎉╎قائمـــة أوامـر تغييـر المنطقـة الزمنيـة للوقـت 🌐:** \n\n"
    "⪼ `.وقت اليمن` \n\n"
    "⪼ `.وقت العراق` \n\n"
    "⪼ `.وقت السعودية` \n\n"
    "⪼ `.وقت سوريا` \n\n"
    ...
    "⪼ `.وقت المغرب` \n\n"
    "⪼ `.وقت تركيا` \n\n"
    "🛃 سيتـم إضـافة المزيـد من الدول قريبًــا\n\n"
...
async def variable(event):
    ...
    if input_str == "كليشة الفحص" or input_str == "كليشه الفحص":
        ...
        if gvarstatus("pmblock") is None:
            await zed.edit("**⎉╎تم تغييـر {} بنجـاح ☑️**\n**⎉╎الكليشـة الجـديده** \n {} \n\n**⎉╎الان قـم بـ تفعيـل حماية الخاص عبر الأمر ↶** ( `الحماية تفعيل` ) **لـ التحقـق مـن الكليشـة . .**".format(input_str, vinfo))
        else:
            await zed.edit("**⎉╎تم تغييـر {} بنجـاح ☑️**\n**⎉╎الكليشـة الجـديده** \n {} \n\n**⎉╎الان قـم بـ تفعيـل حماية الخاص عبر الأمر ↶** ( `الحماية تفعيل` ) **لـ التحقـق مـن الكليشـة . .**".format(input_str, vinfo))
        ...
        if variable in heroku_var:
            await zed.edit("**⎉╎تم تغييـر {} بنجـاح ☑️**\n**⎉╎المتغيـر : ↶**\n `{}` \n**⎉╎يتم الان إعـادة تشغيـل بـوت تيبثـــون يستغـرق الأمر 2-1 دقيقـة ▬▭ ...**".format(input_str, vinfo))
        else:
            await zed.edit("**⎉╎تم إضافـة {} بنجـاح ☑️** \n**⎉╎المضاف إليه :**\n `{}` \n**⎉╎يتم الان إعـادة تشغيـل بـوت تيبثـــون يستغـرق الأمر 2-1 دقيقـة ▬▭ ...**".format(input_str, vinfo))
...
async def variable(event):
    ...
    if input_str == "كليشة الفحص" or input_str == "كليشه الفحص":
        ...
        await zed.edit("**⎉╎تم حـذف {} بنجـاح ☑️**\n**⎉╎المتغيـر المحـذوف : ↶**\n `{}` \n**⎉╎يتم الان إعـادة تشغيـل بـوت تيبثـــون يستغـرق الأمر 2-1 دقيقـة ▬▭ ...**".format(input_str, variable))
        ...
        if variable not in heroku_var:
        	return await zed.edit("**⎉╎عـذࢪاً عـزيـزي .. انت لـم تقـم بإضـافـة فـار {} اصـلاً...**".format(input_str))
        await zed.edit("**⎉╎تم حـذف {} بنجـاح ☑️**\n**⎉╎المتغيـر المحـذوف : ↶**\n `{}` \n**⎉╎يتم الان إعـادة تشغيـل بـوت تيبثـــون يستغـرق الأمر 2-1 دقيقـة ▬▭ ...**".format(input_str, heroku_var[variable]))
...
async def custom_zed(event):
    ...
    if (input_str == "كليشة الحماية" or input_str == "كليشة الحمايه" or input_str == "كليشه الحماية" or input_str == "كليشه الحمايه"):
        ...
        variable = "TZ"
        if variable not in heroku_var:
            await zed.edit("**⎉╎فـار المنطقــة الزمنيـــة غيـر موجـود ❌**\n**⎉╎لـ إضـافته فقـط استخـدم الامـر : ↶**\n `.وقت` واسـم الدولـة \n\n**⎉╎قنـاة السـورس : @Tepthon**")
        else:
            await zed.edit("**⎉╎الفـار {} موجـود ☑️**\n**⎉╎قيمـة الفـار : ↶**\n `{}` \n\n**⎉╎قنـاة السـورس : @Tepthon**".format(input_str, variable))
        ...
        if variable is None:
            await zed.edit("**⎉╎فـار ايموجي/رمز الايدي غيـر موجـود ❌**\n**⎉╎لـ إضـافته بالـرد ع الكليشـة من هنا ( @tepthonkalesh ) استخـدم الامـر : ↶**\n `.اضف كليشة الايدي` \n\n**⎉╎قنـاة السـورس : @Tepthon**")
        else:
            await zed.edit("**⎉╎الفـار {} موجـود ☑️**\n**⎉╎قيمـة الفـار : ↶**\n `{}` \n\n**⎉╎قنـاة السـورس : @Tepthon**".format(input_str, variable))
...
async def variable(event):
    ...
    viran = "Asia/Tehran"
    heroku_var = app.config()
    zed = await edit_or_reply(event, "**⎉╎جـاري أعـداد المنطقــة الزمنيـــة لـ زدثــون 🌐...**")
    # All Rights Reserved for "Zed-Thon" "زلـزال الهيبـه"
    if input_str == "العراق" or input_str == "اليمن" or input_str == "سوريا" or input_str == "السعودية" or input_str == "لبنان" or input_str == "الاردن":
        variable = "TZ"
        await asyncio.sleep(1.5)
        if variable in heroku_var:
            await zed.edit("**⎉╎تم تغييـر المنطقــة الزمنيـــة .. بنجـاح ☑️**\n**⎉╎المتغير : ↶**\n دولـة `{}` \n**⎉╎يتم الان إعـادة تشغيـل بـوت تيبثـــون يستغـرق الأمر 2-1 دقيقـة ▬▭ ...**".format(input_str))
        else:
            await zed.edit("**⎉╎تم إضـافـة المنطقــة الزمنيـــة .. بنجـاح ☑️**\n**⎉╎المضـاف إليه : ↶**\n دولـة `{}` \n**⎉╎يتم الان إعـادة تشغيـل بـوت تيبثـــون يستغـرق الأمر 2-1 دقيقـة ▬▭ ...**".format(input_str))
