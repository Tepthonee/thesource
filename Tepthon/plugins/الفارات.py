...
ZelzalVP_cmd = (
    "𓆩 𝗧𝗲𝗽𝘁𝗵𝗼𝗻 𝗩𝗮𝗿𝘀 **🝢 اوامـر الفـارات** 𓆪\n"
    "**⋆┄─┄─┄─┄─┄─┄─┄─┄⋆**\n"
...
ZelzalTZ_cmd = (
    ...
    "**⋆┄─┄─┄─┄─┄─┄─┄─┄⋆**\n"
    "**⎉╎قائمـة اوامر تغييـر المنطقـة الزمنيـة لـ ضبط الوقت ع تيبثـون حسب توقيت دولتك 🌐:** \n\n"
    "⪼ `.وقت فلسطين` \n"
    "⪼ `.وقت اليمن` \n"
    "⪼ `.وقت العراق` \n"
    "⪼ `.وقت السعودية` \n"
    ...
    "⪼ `.وقت اندنوسيا` \n"
    "⪼ `.وقت ماليزيا` \n\n"
    "**🛃 إذا لم تجد دولتك .. قم بالبحث عن أقرب دولة لها**\n"
    "𓆩 [𝗧𝗲𝗽𝘁𝗵𝗼𝗻 𝗩𝗮𝗿𝘀 - قنـاة الفـارات](t.me/Tepthone1) 𓆪"
)
...
async def variable(event):
    ...
    if input_str == "كليشة الفحص" or input_str == "كليشه الفحص":
        ...
        if gvarstatus("ALIVE_NAME") is not None:
            await zed.edit("**⎉╎تم تغييـر {} بنجـاح ☑️**\n**⎉╎المتغيـر : ↶**\n `{}` \n**⎉╎يتم الآن إعـادة تشغيـل بـوت تيبثـون يستغـرق الأمر 2-1 دقيقـة ▬▭ ...**".format(input_str, vinfo))
        else:
            await zed.edit("**⎉╎تم اضافـة {} بنجـاح ☑️** \n**⎉╎المضاف إليـه :**\n `{}` \n**⎉╎يتم الآن إعـادة تشغيـل بـوت تيبثـون يستغـرق الأمر 2-1 دقيقـة ▬▭ ...**".format(input_str, vinfo))
        addgvar(variable, vinfo)
        ...
        await asyncio.sleep(1.5)
        if vinfo.isdigit():
            await zed.edit("**⎉╎تم تغييـر {} .. بنجـاح ☑️**\n**⎉╎عـدد تحذيـرات حماية الخاص الآن : ↶**\n `{}`".format(input_str, vinfo))
        else:
...
async def variable(event):
    ...
    if input_str == "كليشة الفحص" or input_str == "كليشه الفحص":
        ...
        await zed.edit("**⎉╎تم حـذف {} بنجـاح ☑️**\n**⎉╎المتغيـر المحـذوف : ↶**\n `{}` \n**⎉╎يتم الآن إعـادة تشغيـل بـوت تيبثـون يستغـرق الأمر 2-1 دقيقـة ▬▭ ...**".format(input_str, variable))
        ...
        if variable is None:
        	return await zed.edit("**⎉╎عـذࢪاً عـزيـزي .. انت لـم تقـم باضـافـة فـار {} اصـلاً...**".format(input_str))
        await zed.edit("**⎉╎تم حـذف {} بنجـاح ☑️**\n**⎉╎المتغيـر المحـذوف : ↶**\n `{}` \n**⎉╎يتم الآن إعـادة تشغيـل بـوت تيبثـون يستغـرق الأمر 2-1 دقيقـة ▬▭ ...**".format(input_str, heroku_var[variable]))
        delgvar(variable)
    elif input_str == "رسائل الحماية" or input_str == "رسائل الحمايه" or input_str == "رسائل الخاص" or input_str == "رسائل حماية الخاص" or input_str == "عدد التحذيرات":
        variable = "MAX_FLOOD_IN_PMS"
        await asyncio.sleep(1.5)
...
async def variable(event):
    ...
    if input_str == "العراق" or input_str == "اليمن" or input_str == "سوريا" or input_str == "السعودية" or input_str == "لبنان" or input_str == "الاردن" or input_str == "فلسطين" or input_str == "قطر" or input_str == "الكويت" or input_str == "البحرين":
        ...
        if gvarstatus("T_Z") is not None:
            await zed.edit("**⎉╎تم تغييـر المنطقـة الزمنيـة .. بنجـاح ☑️**\n**⎉╎المتغير : ↶**\n دولـة `{}` \n**⎉╎يتم الآن إعـادة تشغيـل بـوت تيبثـون يستغـرق الأمر 2-1 دقيقـة ▬▭ ...**".format(input_str))
        else:
...
async def _(tiba):
    if tiba.fwd_from:
        return
    rep = await edit_or_reply(tiba, "**⎉╎جـاري اضـافة فـار الصـورة إلى بـوتك ...**")
