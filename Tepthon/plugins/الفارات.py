...
ZelzalVP_cmd = (
    ...
    "⪼ `.وقت ايطاليا` \n\n"
    "⪼ `.وقت امريكا` \n\n"
    "🛃 سيتـم إضافة المزيـد من الدول قريبًا\n\n"
...
ZelzalTZ_cmd = (
    ...
    "⪼ `.وقت ايطاليا` \n\n"
    "⪼ `.وقت امريكا` \n\n"
    "🛃 سيتـم إضافة المزيـد من الدول قريبًـا\n\n"
    "\n𓆩 [𝗧𝗲𝗽𝘁𝗵𝗼𝗻 𝗩𝗮𝗿𝘀 - قنـاة الفـارات](t.me/Tepthone1) 𓆪"
)
...
async def variable(event):
    ...
    if input_str == "كليشة الفحص" or input_str == "كليشه الفحص":
        ...
        await asyncio.sleep(1.5)
        if vinfo.isdigit():
            await zed.edit("**⎉╎تم اضافـة {} بنجـاح ☑️**\n**⎉╎الثوانـي الجـديـدة** \n {} \n\n**⎉╎الان قـم بـ ارسـال الأمــر ↶** `.لانهائي` **+ اسم بوت التجميـع لـ البـدء بالتجميـع اللانهائـي . .**".format(input_str, vinfo))
        else:
            return await zed.edit("**⎉╎خطـأ .. قم بالـرد ع رقـم فقـط ؟!**")
        addgvar(variable, vinfo)
...
async def variable(event):
    ...
    if input_str == "كليشة الفحص" or input_str == "كليشه الفحص":
        ...
        await zed.edit("**⎉╎تم حـذف {} بنجـاح ☑️**\n**⎉╎المتغيـر المحـذوف : ↶**\n `{}` \n**⎉╎يتم الان اعـادة تشغيـل بـوت تيبثـــون يستغـرق الامر 2-1 دقيقــة ▬▭ ...**".format(input_str, variable))
        ...
        if variable is None:
        	return await zed.edit("**⎉╎عـذࢪاً عـزيـزي .. انت لـم تقـم باضـافـة فـار {} اصـلاً...**".format(input_str))
        await zed.edit("**⎉╎تم حـذف {} بنجـاح ☑️**\n**⎉╎المتغيـر المحـذوف : ↶**\n `{}` \n**⎉╎يتم الان اعـادة تشغيـل بـوت تيبثـــون يستغـرق الامر 2-1 دقيقــة ▬▭ ...**".format(input_str, heroku_var[variable]))
        delgvar(variable)
    elif input_str == "رسائل الحماية" or input_str == "رسائل الحمايه" or input_str == "رسائل الخاص" or input_str == "رسائل حماية الخاص" or input_str == "عدد التحذيرات":
        variable = "MAX_FLOOD_IN_PMS"
        await asyncio.sleep(1.5)
...
async def custom_zed(event):
    ...
    if (input_str == "كليشة الحماية" or input_str == "كليشة الحمايه" or input_str == "كليشه الحماية" or input_str == "كليشه الحمايه"):
        ...
        variable = gvarstatus("START_BUTUN")
        if variable is None:
            await zed.edit("**⎉╎فـار زر كليشـة ستـارت البـوت غيـر موجـود ❌**\n**⎉╎لـ اضـافته بالـرد ع يـوزرك او يـوزر قناتـك استخـدم الأمــر : ↶**\n `.اضف فار زر الستارت` \n\n**⎉╎قنـاة السـورس : @Tepthon**")
        else:
            await zed.edit("**⎉╎الفـار {} موجـود ☑️**\n**⎉╎قيمـة الفـار : ↶**\n https://t.me/{}\n\n**⎉╎قنـاة السـورس : @Tepthon**".format(input_str, variable))
        ...
        variable = "OWNER_ID"
        if variable not in heroku_var:
            await zed.edit("**⎉╎فـار ايـدي الحسـاب غيـر موجـود ❌**\n**⎉╎لـ اضـافته بالـرد ع الايـدي فقـط استخـدم الأمــر : ↶**\n `.اضف فار ايدي الحساب` \n\n**⎉╎قنـاة السـورس : @Tepthon**")
        else:
            await zed.edit("**⎉╎الفـار {} موجـود ☑️**\n**⎉╎قيمـة الفـار : ↶**\n `{}` \n\n**⎉╎قنـاة السـورس : @Tepthon**".format(input_str, variable))
        ...
        variable = "TG_BOT_TOKEN"
        if variable not in heroku_var:
            await zed.edit("**⎉╎فـار توكـن البـوت غيـر موجـود ❌**\n**⎉╎لـ اضـافته بالـرد ع التوكـن فقـط استخـدم الأمــر : ↶**\n `.اضف فار التوكن` \n\n**⎉╎قنـاة السـورس : @Tepthon**")
        else:
            await zed.edit("**⎉╎الفـار {} موجـود ☑️**\n**⎉╎قيمـة الفـار : ↶**\n `{}` \n\n**⎉╎قنـاة السـورس : @Tepthon**".format(input_str, variable))
        ...
        variable = "UPSTREAM_REPO"
        if variable not in heroku_var:
            await zed.edit("**⎉╎فـار الريبـو غيـر موجـود ❌**\n**⎉╎لـ اضـافته بالـرد ع رابط السورس الرسمي استخـدم الأمــر : ↶**\n `.اضف فار الريبو` \n\n**⎉╎قنـاة السـورس : @Tepthon**")
        else:
            await zed.edit("**⎉╎الفـار {} موجـود ☑️**\n**⎉╎قيمـة الفـار : ↶**\n `{}` \n\n**⎉╎قنـاة السـورس : @Tepthon**".format(input_str, variable))
        ...
        variable = "T_Z"
        if variable not in heroku_var:
            await zed.edit("**⎉╎فـار المنطقــة الزمنيـة غيـر موجـود ❌**\n**⎉╎لـ اضـافته فقـط استخـدم الأمــر : ↶**\n `.وقت` واسـم الدولـة \n\n**⎉╎قنـاة السـورس : @Tepthon**")
        else:
            await zed.edit("**⎉╎الفـار {} موجـود ☑️**\n**⎉╎قيمـة الفـار : ↶**\n `{}` \n\n**⎉╎قنـاة السـورس : @Tepthon**".format(input_str, variable))
        ...
        if variable is None:
            await zed.edit("**⎉╎فـار ايموجي/رمز الايدي غيـر موجـود ❌**\n**⎉╎لـ اضـافته بالـرد ع الكليشـة من هنا ( @tepthonkalesh ) استخـدم الأمــر : ↶**\n `.اضف كليشة الايدي` \n\n**⎉╎قنـاة السـورس : @Tepthon**")
        else:
            await zed.edit("**⎉╎الفـار {} موجـود ☑️**\n**⎉╎قيمـة الفـار : ↶**\n `{}` \n\n**⎉╎قنـاة السـورس : @Tepthon**".format(input_str, variable))
...
async def variable(event):
    ...
    zed = await edit_or_reply(event, "**⎉╎جـاري إعداد المنطقــة الزمنيـة لـ تيبثـــون 🌐...**")
    # All Rights Reserved for "Zed-Thon" "زلـزال الهيبـه"
    if input_str == "العراق" or input_str == "اليمن" or input_str == "سوريا" or input_str == "السعودية" or input_str == "لبنان" or input_str == "الاردن":
        ...
        if gvarstatus("T_Z") is not None:
            await zed.edit("**⎉╎تم تغييـر المنطقــة الزمنيـة .. بنجـاح ☑️**\n**⎉╎المتغير : ↶**\n دولـة `{}` \n**⎉╎يتم الان اعـادة تشغيـل بـوت تيبثـــون يستغـرق الامر 2-1 دقيقــة ▬▭ ...**".format(input_str))
        else:
            await zed.edit("**⎉╎تم اضـافـة المنطقــة الزمنيـة .. بنجـاح ☑️**\n**⎉╎المضـاف اليـة : ↶**\n دولـة `{}` \n**⎉╎يتم الان اعـادة تشغيـل بـوت تيبثـــون يستغـرق الامر 2-1 دقيقــة ▬▭ ...**".format(input_str))
        addgvar("T_Z", viraq)
